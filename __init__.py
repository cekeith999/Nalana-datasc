...
_LIVE_ACTION_QUEUE = queue.Queue()  # Legacy (thread-based Live); kept for compatibility
_LIVE_SESSION_RUNNING = False  # True while Live subprocess is running
_LIVE_LISTENER = None  # Legacy placeholder (no longer used for Gemini Live)
_LIVE_TIMER = None  # Legacy placeholder (no longer used for Gemini Live)
...
class STB_OT_LiveStart(bpy.types.Operator):
    ...
    def execute(self, context):
        global _LIVE_SESSION_RUNNING, _LIVE_PROCESS
        if _LIVE_SESSION_RUNNING and _LIVE_PROCESS is not None and _LIVE_PROCESS.poll() is None:
            self.report({'WARNING'}, "Live session already running")
            return {'CANCELLED'}

        # Get Gemini API key from addon preferences
        try:
            addon_prefs = context.preferences.addons[ADDON_ROOT].preferences
        ...
        addon_dir = os.path.dirname(os.path.abspath(__file__))
        live_script = os.path.join(addon_dir, "live_bridge.py")
        if not os.path.isfile(live_script):
            self.report({'ERROR'}, f"Live runner script not found: {live_script}")
            return {'CANCELLED'}
        ...
        try:
            ...
            if os.name == "nt":
                ...
                with open(batch_file, 'w') as f:
                    ...
                    f.write(f'"{python_exe}" "{live_script}"\n')
                    f.write(f'if errorlevel 1 (\n')
                    f.write(f'    echo.\n')
                    f.write(f'    echo Live runner exited with error code %errorlevel%\n')
                    f.write(f'    pause\n')
                    f.write(f')\n')
                
                # Launch batch file with new console window
                ...
                # Non-Windows: just run directly
                _LIVE_PROCESS = subprocess.Popen(
                    [python_exe, live_script],
                    cwd=addon_dir or None,
                    env=env,
                )
            ...
            _LIVE_PROCESS = None
            _LIVE_SESSION_RUNNING = False
            self.report({'ERROR'}, f"Failed to start Live runner: {e}")
            import traceback
            traceback.print_exc()
        return {'FINISHED'}
...
class STB_OT_LiveStop(bpy.types.Operator):
    ...
    def execute(self, context):
        global _LIVE_SESSION_RUNNING, _LIVE_PROCESS
        _LIVE_SESSION_RUNNING = False
        if _LIVE_PROCESS is not None:
            try:
                _LIVE_PROCESS.terminate()
                _LIVE_PROCESS.wait(timeout=3)
            except Exception:
                try:
                    _LIVE_PROCESS.kill()
                except Exception:
...
class STB_PT_Live(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        global _LIVE_SESSION_RUNNING, _LIVE_PROCESS
        is_running = _LIVE_SESSION_RUNNING and (_LIVE_PROCESS is not None) and (_LIVE_PROCESS.poll() is None)
...
def unregister():
    ...
    # Stop Live session if running (Phase 7 - Gemini Live subprocess)
    try:
        global _LIVE_SESSION_RUNNING, _LIVE_PROCESS
        _LIVE_SESSION_RUNNING = False
        if _LIVE_PROCESS is not None:
