...
# Expanded local rules for common operations to improve offline performance and serve as seed data.
EXPANDED_LOCAL_RULES = {
    "object.modifier_add(type='SUBSURF')": [
        "make it smooth", "subdivide it", "add smoothness", "smooth out the mesh", "give it more detail",
        "smooth modifier", "render smooth", "high poly look", "increase resolution", "smooth surfaces",
        "add subdivision modifier", "make it look high-res", "smooth out those edges", "give it some subdivisions",
        "apply smoothing", "make the object smooth", "subdivide selection", "add more geometry for smoothness",
        "smooth shading modifier", "make it rounded and smooth"
    ],
    "object.modifier_add(type='BEVEL')": [
        "round the edges", "bevel corners", "make edges soft", "add bevel", "fillet the edges",
        "chamfer", "soften corners", "edge roundness", "bevel modifier", "smooth edges",
        "round out the corners", "add a bevel to this", "make it look less sharp", "bevel selection",
        "sand down the edges", "add some edge wear", "rounded look", "bevel the boundary",
        "soften those sharp bits", "add a beveling effect"
    ],
    "mesh.extrude_region_move": [
        "extrude this", "pull it out", "push it in", "lengthen the face", "grow the selection",
        "extrude region", "offset face", "push pull", "expand face", "extrude along normal",
        "pull out the face", "push in the geometry", "make it longer from here", "extrude faces",
        "pull that part forward", "push that part back", "grow the mesh out", "extrude and move",
        "move selection outward", "pull the face away"
    ],
    "object.shade_smooth": [
        "shade smooth", "look smooth", "don't show faces", "hide polygons", "smooth shading",
        "make it look organic", "remove facets", "smooth it", "shade it round", "organic look",
        "hide the wireframe look", "make it look like a single piece", "smooth surface look",
        "no more blocky look", "make it look realistic", "smooth out the shading", "faceted to smooth",
        "make it look curved", "remove the low poly look", "smooth out the normals"
    ],
    "object.join": [
        "join objects", "combine", "union", "merge selected objects", "make them one",
        "weld together", "group into one mesh", "join these two", "combine selection",
        "bridge into one object", "stick them together", "merge objects", "join all selected",
        "make a single object", "combine meshes", "fuse objects", "join into active",
        "weld selection", "combine into one", "join them up"
    ],
    "object.parent_set": [
        "parent this to that", "make child", "attach to", "set parent", "link as child",
        "parent selection", "make this the parent", "attach this object to the other one",
        "bind to parent", "set relationship", "make this follow that", "parent with keeping transform",
        "link objects hierarchically", "set up parenting", "make a child object", "nest this under that",
        "attach to active", "parent selected to active", "make it follow", "setup parent child link"
    ],
    "object.modifier_add(type='MIRROR')": [
        "mirror it", "symmetry", "reflect", "add mirror modifier", "make it symmetrical",
        "flip it to the other side", "mirror along axis", "add symmetry modifier", "reflect the mesh",
        "duplicate on the other side", "mirror across", "enable mirroring", "copy to other side",
        "symmetry mode", "mirror selection", "reflect over x", "make a mirrored copy",
        "add a mirror", "set up reflection", "symmetrical modeling"
    ],
    "mesh.inset_faces": [
        "inset faces", "offset inward", "create border inside", "inset selection", "pull edges in",
        "make a frame inside", "inset this face", "offset face inward", "create inner border",
        "inset the plane", "shrink face inside", "inset geometry", "make a sub-face",
        "inset inward", "create a border", "offset edges", "inset the boundary",
        "create a pocket", "inset the selection", "offset faces"
    ],
    "mesh.loopcut_slide": [
        "add loop cut", "edge loop", "cut it across", "slice through", "add a ring cut",
        "loop cut and slide", "add more edges in a loop", "cut the mesh horizontally",
        "add a vertical cut", "slice the object", "add a new edge loop", "ring cut",
        "cut through the middle", "add geometry loop", "slice it open", "cut the mesh",
        "add a seam cut", "divide the mesh with a loop", "edge ring cut", "split the faces with a loop"
    ],
    "view3d.toggle_xray": [
        "see through", "toggle xray", "transparent mode", "look inside", "ghost mode",
        "see through the mesh", "xray vision", "toggle transparency", "look at back faces",
        "see through objects", "show hidden parts", "xray view", "transparent view",
        "see what's behind", "toggle xray display", "show occlusion", "see inside the object",
        "xray mode on", "look through it", "toggle mesh transparency"
    ]
}
...
def try_local_rules(text: str):
    ...
    # Expanded operations check
    for op_spec, patterns in EXPANDED_LOCAL_RULES.items():
        if _match_any_phrase(tl, patterns):
            if "(" in op_spec:
                # Handle ops with hardcoded kwargs like object.modifier_add(type='SUBSURF')
                base_op, kw_str = op_spec.split("(", 1)
                kw_str = kw_str.rstrip(")")
                # Extremely primitive kwarg parsing for the known fixed values
                kwargs = {}
                for pair in kw_str.split(","):
                    if "=" in pair:
                        k, v = pair.split("=")
                        kwargs[k.strip()] = v.strip().strip("'").strip('"')
                return {"op": base_op, "kwargs": kwargs}
            return {"op": op_spec, "kwargs": {}}
